...
-- 연습문제 01 : FIRST_NAME이 Neena인 사원과 같은 도시에서 근무하는 모든 사원의 이름과 부서번호를 조회해보세요 
...
-- 연습문제 02 : FIRST_NAME이 Neena인 사원과 급여가 5000이상 차이나지 않는 모든 사원을 조회해보세요
SELECT 
    e1.*
    FROM
    employees e1
    INNER JOIN employees e2 ON e2.first_name = 'Neena' AND ABS(e2.salary- e1.salary) <= 5000
...
--WHERE
--    first_name = 'Neena');
--   'Neena' <= '5000' OR 'Neena' >= 5000;
    
-- 연습문제 03 : 시애틀에서 근무하는 모든 사원의 이름/부서명/도시를 조회
...
WHERE
            city = 'Seattle';
-- 연습문제 04 : 매니저의 이름이 King인 모든 사원의 이름/직책/연봉을 조회
...
FROM
    employees e1
    INNER JOIN employees e2 ON e1.manager_id = e2.employee_id AND e2.last_name ='King'
