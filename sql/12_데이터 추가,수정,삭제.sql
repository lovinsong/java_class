...
-- # 가장 기본적인 데이터 추가
INSERT INTO coffees(cid, cname, cprice) VALUES(1, '아이스 아메리카노', 2000);
INSERT INTO coffees(cname, cid, cprice) VALUES('뜨거운 아메리카노', 2, 1500);

-- # 일부 컬럼에만 데이터 추가
INSERT INTO coffees(cname, cprice) VALUES('아이스 카페라뗴', 2500);
INSERT INTO coffees(cprice, cid) VALUES(3000,5);

-- # 컬러을 생략하면서 데이터 추가 (모든 컬럼에 순서대로 모든 값을 넣어야 함)
...
-- 연습문제 01: employees 테이블의 구조만 복사하는 쿼리문을 이용하여 emp 테이블을 생성해보세요
CREATE TABLE emp AS SELECT * FROM employees WHERE 1 = 0;

SELECT * FROM emp;

-- 연습문제 02: emp테이블에 employees의 모든 사원들 중 홀수 사번을 가진 사원들만 추가해보세요
...
-- 연습문제 03; emp테이블의 모든 사원들 중 JOB_ID에 MAN이 포함된 사원들의 월급을 40% 삭감 해보세요
