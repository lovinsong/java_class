...
-- 월급이 15000 이상인 사원의 이름 조회하기
SELECT last_name AS 부자 FROM employees WHERE salary >= 15000;
...
-- 문자열을 나타낼때는 리터럴을 나타낼때는 반드시 ''를 사용해야 한다
...
-- 연습문제01 : 2000에서 3000사이의 월급을 받는 사원을 조회해보세요
SELECT * FROM employees WHERE 2000 <= salary AND salary <= 3000;
SELECT * FROM employees WHERE salary BETWEEN 2000 AND 3000;

-- 연습문제02 : 부서번호가 30번이거나 60번이거 90번인 사원들의 모든 정보를 조회해보세요
...
-- 핸드폰 번호에 .123.이 포함된 모든 튜플을
SELECT * FROM employees WHERE phone_number LIKE '%.123.%'; 
SELECT * FROM employees WHERE phone_number LIKE '%123.%'; 

-- LAST_NAME의 두 번째 글자가 a인 모든 행을
SELECT * FROM employees WHERE last_name LIKE '_a%';

-- LAST_NAME의 a가 두 개 이상 포함된 모든 행을 조회
SELECT * FROM employees WHERE last_name LIKE '%a%a%'; 

-- 연습 문제 04 : LAST_NAME의 뒤에서 세 번째 글자가 a인 모든 사원을 조회해보세요
SELECT * FROM employees WHERE last_name LIKE '%a__'; 

-- 연습 문제 05 : LAST_NAME의 3번째 글자가 r이고 핸드폰 번호에 .이 3개 포함된 모든 사원을 조회해보세요
