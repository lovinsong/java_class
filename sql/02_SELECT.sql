...
/*
    ...
    # CHAR(n)
        ...
        - 설정된 길이보다 적은 문자 데이터가 들어오더라도
          고정적으로 설정된 길이를 차지한다
        - CHAR(10)로 설정된 컬럼에 2글자가 들어오더라도 10바이트를 차지한다
    # VARCHAR2(n)
        - 가변 길이 문자 데이터를 저장할 때 사용하는 컬럼 타입
        - 저장된 데이터의 크기에 따라 알맞은 공간만 사용한다
        - 저장 공간의 낭비를 줄일 수 있다
        - VARCHAR2(10)로 설정된 컬럼에 2글자가 들어오면 2바이트만 차지한다
    # DATE
        - 날짜 및 시간을 저장하는 컬럼 타입
...
SELECT last_name, commission_pct * salary AS 보너스 FROM employees;

-- 월급 + 보너스 계산해보기
-- ※ null은 존재하지 않는 값이기 때문에 산술연산에 포함되면 무조건 null이 된다
SELECT last_name, salary + commission_pct * salary AS 실수령액 FROM employees;

-- NVL(컬럼명, 대체값) 함수 : 해당 컴럼에서 NULL값이 나왔을 때 대체값을 사용한다
SELECT
...
-- 연습문제02 : 모든 사원들의 사번/이름/연봉을 출력해보세요(참고로 salary는 월급, 연봉 컬럼에는 한글 별칭을 사용)
SELECT employee_id AS 사번,last_name AS 이름, salary * (NVL(commission_pct, 0) + 1) * 12 AS "연봉($)" FROM employees;
...
-- 매니저 역할을 하고 있는 사원들의 사번을 한번씩만 조회 할 수 있다
