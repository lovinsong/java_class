...
public class Clazz {
	...
	public static void main(String[] args) throws NoSuchMethodException, SecurityException, ClassNotFoundException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoMoreCalorieException {
		...
		System.out.println("forName으로 불러온 simpleName이름 : " + c.getSimpleName());
		...
		try {
			...
			System.out.println("c3가 인터페이스 인가요? " + c3.isInterface());
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		}

		// Class 타입 변수에 저장 된 클래스들은 설계도 형태로 존재하고 있다
		// 	설계도를 빼오는 방법 1 : forName메서드에 패키지명을 정확하게 입력한다
		//	설계도를 빼오는 방법 2 : 만들어진 인스턴스에서 getClass()메서드를 실행한다
		
		// 이 클래스에 어떤 생성자들이 있는지 알아내는 방법
		for(Constructor con : c.getConstructors()) {
			System.out.println(con.getName());
			// Constructor.getParameterTypes()
			//	- 해당 생성자가 전달받아야 하는 매개변수들을 배열 형태로 반환한다
